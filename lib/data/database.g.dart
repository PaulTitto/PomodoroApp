    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Setting` (`id` INTEGER NOT NULL, `name` TEXT NOT NULL, `value` TEXT NOT NULL, PRIMARY KEY (`id`))');
...
class _$SettingDao extends SettingDao {
  ...
  @override
  Stream<Setting?> insertSettingPomodoro(
    String name,
    String value,
  ) {
  ...
  @override
  Stream<Setting?> findSettingById(
    String name,
    String value,
  ) {
    return _queryAdapter.queryStream(
        'UPDATE Setting SET value = ?2 WHERE name = ?1',
        mapper: (Map<String, Object?> row) => Setting(
            row['id'] as int, row['name'] as String, row['value'] as String),
        arguments: [name, value],
        queryableName: 'no_table_name',
